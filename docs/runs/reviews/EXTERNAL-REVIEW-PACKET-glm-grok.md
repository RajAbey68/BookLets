# EXTERNAL ADVERSARIAL REVIEW PACKET — FABLE5 BookLets run
# For: GLM 5.2 and Grok (run each brief through both; paste verdicts back to Fable)
# Generated 2026-07-12 18:40 UTC

## YOUR ROLE
You are an independent adversarial reviewer with NO prior context. An autonomous Claude orchestrator ('Fable') ran a 12-service build (S1-S12→Z) on this repo. Your job: try to REFUTE its progress claims. Be hostile to hand-waving. A PASS from you is a signed gate; only give it if you cannot break the claim.

## THE RUN'S OWN ACCOUNT (bus)
```
---
...
## CLAIMS TO ATTACK (verify each against the diffs below)
1. conf-gate (#73): automated OCR entries CANNOT be POSTED at the type level — no POSTED branch exists in gateAutomatedJournalEntry.
2. rls-lock (#76): setRlsOrgContext(tx, organizationId) passes explicit org id at all 6 interactive-transaction sites; FORCE RLS is staged Phase-2, not applied.
3. zip-ingest (#75): path-traversal / zip-bomb / >1000-entry / >200MB guards all run BEFORE any OCR spend; body streams through a byte-cap.
4. phantom-fix (#77): D4 already fixed by RAJ-287 — manual booking posts DR Cash / CR Guest Pre-payments, never revenue.

## THE MISSION'S REAL 'DONE' BAR (spec §1) — how far is the run from it?
...
## DIFF STATS PER SERVICE BRANCH (against main)
### claude/fable5-doc-drift
 AGENTS_LOG.md                       | 32 ++++++++++++++----
 ROADMAP.md                          | 65 +++++++++++++++++++------------------
 docs/BRIEFING_FOR_OTHER_SERVICES.md | 29 ++++++++++++-----
 3 files changed, 79 insertions(+), 47 deletions(-)

### claude/s4-conf-gate
 AGENTS_LOG.md                              |  32 ++++
 src/components/ReceiptUploader.tsx         |   2 +-
 src/lib/approval.service.ts                |  36 +++++
 src/lib/automation.service.ts              |  35 ++++-
 tests/unit/receipt-confidence-gate.test.ts | 238 +++++++++++++++++++++++++++++
 5 files changed, 337 insertions(+), 6 deletions(-)

### claude/s2-deploy-fix
 AGENTS_LOG.md               |  33 ++++++++++-
 middleware.ts               |  41 --------------
 src/app/api/health/route.ts |  13 ++++-
 src/auth.config.ts          |   9 +--
 src/proxy.ts                | 132 ++++++++++++++++++++++++++++++++++++++++++++
 6 files changed, 193 insertions(+), 50 deletions(-)

### claude/s5-zip-ingest
 src/app/api/ingest/zip/route.ts     | 140 +++++++++
 src/lib/zip-ingest.deps.ts          |  74 +++++
 src/lib/zip-ingest.ts               | 567 ++++++++++++++++++++++++++++++++++++
 tests/unit/zip-ingest-route.test.ts | 168 +++++++++++
 tests/unit/zip-ingest.test.ts       | 519 +++++++++++++++++++++++++++++++++
 8 files changed, 1530 insertions(+)

### claude/s3-rls-lock
 tests/unit/journal-optimistic-lock.test.ts         |   4 +-
 tests/unit/journal-reverse-entry.test.ts           |   1 +
 tests/unit/ledger-postentry-tx-reuse.test.ts       | 255 +++++++++++++++
 tests/unit/org-context.test.ts                     | 177 +++++++++++
 tests/unit/rls-org-isolation.test.ts               | 246 +++++++++++++++
 16 files changed, 1679 insertions(+), 28 deletions(-)

### claude/s6-review-ui
 src/components/DraftReviewQueue.tsx       | 364 ++++++++++++++++++++++++++++++
 src/lib/approval.service.ts               |  13 ++
 src/lib/draft-evidence.ts                 |  61 +++++
 tests/unit/batch-approval-actions.test.ts | 300 ++++++++++++++++++++++++
 tests/unit/draft-evidence.test.ts         |  45 ++++
 8 files changed, 1090 insertions(+), 61 deletions(-)

### claude/s10-phantom-fix
 AGENTS_LOG.md                                    |  25 ++
 tests/unit/booking-phantom-revenue-fence.test.ts | 303 +++++++++++++++++++++++
 2 files changed, 328 insertions(+)

## VERDICT FORMAT (reply exactly)
VERDICT: PASS | BLOCK  (per claim 1-4, plus overall)
checkerIdentity: <GLM-5.2 | Grok + version>
FALSE/EXAGGERATED CLAIMS FOUND: <list with evidence, or 'none'>
BIGGEST LIVE-DEPLOY RISK: <one thing most likely to break when this hits the real DB/prod>
GRADE: integrity <A-F>, progress-efficiency <A-F>
